...
git -C "$ROOT_DIR" ls-files -z | tar --null -T - -czf "$DEPLOY_ARCHIVE"
...
cd "$REMOTE_ROOT"
docker compose -f infra/docker-compose.yml up -d --build backend nginx
